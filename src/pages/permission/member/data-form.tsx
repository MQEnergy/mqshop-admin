...
export function DataForm<TData>({...props}: DataFormProps<TData>) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: props.data ? info as DefaultValues<any> : defaultValues
  })
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    const runAsync = createRes.runAsync({
    ...
    await toast.promise(
        runAsync,
        {
          ...
          error: (err) => <text
              className='text-sm'>{err.response?.data.message || err.message || 'Request Error'}</text>,
        }
    )
  ...
  return (
      <DrawerForm open={props.open} onClose={handleClose} onSubmit={form.handleSubmit(onSubmit)} loading={createRes.loading}
        ...
        <Form {...form}>
          <div className='grid gap-2'>
            <FormField
                ...
                name='avatar'
                render={({}) => (
                    <FormItem className='flex items-center space-b-4'>
