...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, DrawerContentProps>(
    ({className, children, showBar = true, ...props}, ref) => (
        <DrawerPortal>
          ...
          >
            {showBar ? <div
                className='mx-auto mt-4 h-[100px] w-2 rounded-full bg-muted ml-2 absolute top-1/2 transform -translate-y-1/2'/> : null}
            {children}
          </DrawerPrimitive.Content>
        </DrawerPortal>
    ),
...
export function DrawerForm(props: DrawerFormProps) {
  return (
      <Drawer open={props.open || false} direction={props.direction || 'right'}>
        <DrawerContent className='top-0 right-0 left-auto mt-0 w-[500px] rounded-none' showBar={props.showBar}>
          <div className='mx-auto w-full overflow-y-scroll overflow-x-hidden h-screen'>
            ...
            <DrawerFooter className='w-full bg-background h-[70px] absolute bottom-0 left-0 flex-row border-t'>
              <Button onClick={props.onSubmit} className='w-[100px]'>{props.submitTitle || '提交'}</Button>
              <DrawerClose asChild>
...
//             </div>
//             <DrawerFooter>
//               <Button onClick={props.onSubmit}>{props.submitTitle || '提交'}</Button>
//               <DrawerClose asChild>
//                 <Button variant="outline" onClick={props.onClose}>取消</Button>
//               </DrawerClose>
//             </DrawerFooter>
